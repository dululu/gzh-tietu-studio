...
- 图多的时候只列必须署名的，许可名合并成一组——`foot` **最多 3 行**（一行约 41 个汉字 / 76 个西文字符），
  超了会折行、把内容区顶小。
- **CC BY-SA 的图有「传染性」**：「相同方式共享」意味着裁一刀就算改编，改编稿要以同样许可发布。
  本工具统一在 `foot` 里补一行说明：
  - 单张 BY-SA → `经裁剪改编，本图按相同许可 CC BY-SA 3.0 发布`
  - 多张混许可 → 并进许可行，写 `…；BY-SA 图经裁剪，按相同许可发布`

  能选就优先 CC BY / CC0 / 公有领域；商业投放、品牌稿改走 Pexels 或自采图。
...
> 这三张在示例输出里都被裁剪过，示例数据的 `foot` 已按上面的写法标注。你若要再用它们，请一并遵守。
