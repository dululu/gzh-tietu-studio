...
所以做 13 种版式不需要写 13 个模板，改一个字段就切换。

- **13 种版式**：单数字冲击 / 编号清单 / 左右对比 / 对峙对比 / 编号行动卡 / 四步阶梯 /
...
> **说明书里有什么**：环境准备 · 13 种版式逐个讲 · 三个图源怎么找图 · 全字段速查表 · 导出可编辑 SVG · 总览图 · 常见问题 · 踩坑清单
>
...
**13 种版式样板集**（同一套骨架，只换内容区，含配图与四宫格图片版）——
...
做自己的图：复制 [`examples/数据-样板集.json`](examples/数据-样板集.json)（15 张，覆盖全部 13 种版式）
...
├── docs/
│   └── 贴图排版手册.md         # 进阶：排版原理、骨架常量、海报化视觉手段、14 个坑
└── examples/
    ├── 数据-样板集.json         # ★ 15 张、覆盖全部 13 种版式（含两个 hero 选图版本）
    ├── 数据-九种版式示例.json
    ├── 数据-热点示例.json
