...
所以做 15 种版式不需要写 15 个模板，改一个字段就切换。

- **15 种版式**：单数字冲击 / 编号清单 / 左右对比 / 对峙对比 / 编号行动卡 / 四步阶梯 /
...
- **三道自检**：出图自带体检 + 越界探针 + 海报探针，不靠"应该没问题"
...
> **说明书里有什么**：环境准备 · 15 种版式逐个讲 · 三个图源怎么找图 · 全字段速查表 · 导出可编辑 SVG · 总览图 · 常见问题 · 踩坑清单
>
...
**15 种版式样板集**（同一套骨架，只换内容区，含配图与四宫格图片版）——
...
做自己的图：复制 [`examples/数据-样板集.json`](examples/数据-样板集.json)（17 张，覆盖全部 15 种版式）
...
├── 越界探针.js                 # 量页面里每个元素的 rect，抓「看得见地顶出卡片」
├── 海报探针.js                 # 读 computedStyle，确认字号/色块/倾斜真的生效
├── vendor/echarts.min.js      # 图表版式依赖，别删
├── docs/
│   ├── 贴图排版手册.md         # 进阶：排版原理、骨架常量、海报化视觉手段、18 个坑
│   └── 视觉海报案例学习笔记.md  # 案例拆解 → 手法 → 落到哪个版式 → 怎么验
└── examples/
    ├── 数据-样板集.json         # ★ 17 张、覆盖全部 15 种版式（含两个 hero 选图版本）
    ├── 数据-九种版式示例.json
    ├── 数据-热点示例.json
