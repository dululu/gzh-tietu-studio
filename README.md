...
**17 种版式样板集**（同一套骨架，只换内容区，含配图、四宫格图片版、满幅封面与正负形圆形分割）——
...
做自己的图：复制 [`examples/数据-样板集.json`](examples/数据-样板集.json)（20 张，覆盖全部 17 种版式）
...
└── examples/
    ├── 数据-样板集.json         # ★ 20 张、覆盖全部 17 种版式（含两个 hero 选图版本）
    ├── 数据-九种版式示例.json
    ├── 数据-热点示例.json
