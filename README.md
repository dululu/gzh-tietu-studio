...
所以做 17 种版式不需要写 17 个模板，改一个字段就切换。

**另外还有一层开关**：`"shell": "full"` 会拆掉白卡、让底图铺满整张画布 —— **17 个版式都能加**。
这是"信息图"和"海报"之间的那道门。

- **17 种版式**：单数字冲击 / 编号清单 / 左右对比 / 对峙对比 / 编号行动卡 / 四步阶梯 /
  时间轴 / 双轴柱线图 / 横向条形 / 单图 / 圆形焦点 / 双主体+四宫格 / 大图压标题 /
  巨型字主视觉 / 横向色带分割 / 满幅封面 / **正负形圆形分割**
...
> **说明书里有什么**：环境准备 · 17 种版式逐个讲 · 满幅外壳怎么开 · 三个图源怎么找图 · 全字段速查表 · 导出可编辑 SVG · 总览图 · 常见问题 · 踩坑清单
>
...
**版式样板集 15 张**（同一套骨架，只换内容区，含配图、四宫格图片版与满幅封面）——
...
做自己的图：复制 [`examples/数据-样板集.json`](examples/数据-样板集.json)（15 张，覆盖 14 种版式）
...
├── docs/
│   ├── 贴图排版手册.md         # 进阶：排版原理、骨架常量、满幅外壳、海报化视觉手段、24 个坑
│   └── 视觉海报案例学习笔记.md  # 案例拆解 → 手法 → 落到哪个版式 → 怎么验
└── examples/
    ├── 数据-样板集.json         # ★ 15 张、覆盖 14 种版式（含两个 hero 选图版本；photoFocus / band / circle 暂无样张）
    ├── 数据-九种版式示例.json
    ├── 数据-热点示例.json
...
- **2026-09-17 · 样板集 20 → 15 张（按用户评审撤下 5 张）** 用户看总览图逐张过目，给出的评审是
  **「17 很好，18 19 20 21 22 不要」**（总览每张卡都带编号，编号即数据里的文件名前缀）。
  于是**保留** `17_专业撤并这三年_满幅封面_蓝白`（`cover`），**撤下** 5 张：
  `18_` 满幅色带（`band`）、`19_` 满幅巨型字（`bigword`）、`20_` 圆形焦点（`photoFocus`）、
  `21_`/`22_` 正负形圆（`circle` 卡内 / 满幅）。
  - **只动样板集，不动版式代码**：17 种版式、满幅外壳、全部探针都原样保留，随时可以再出样张。
  - 代价是**样板集不再覆盖 `photoFocus` / `band` / `circle` 这三个版式**（它们各自只剩这一张样张，
    撤掉即归零）：**20 张 → 15 张、17 版式 → 14 版式**。文档里的覆盖声明已同步改口径。
  - 老 15 张 PNG 重新渲染后 **md5 全同**：`贴图模板.html` / `导出SVG.js` / `批量出图.js` / `度量.js`
    四个文件与上一个 commit **逐字节相同**，同一份 15 条数据**复跑一遍 15/15 md5 一致**
    —— 只删数据、没碰渲染。
  - 撤下的 5 条数据仍在 git 历史里（上一个 commit 的 `examples/数据-样板集.json`），需要时可直接取回。
- **2026-09-17 · 版式 16 → 17：`circle` 正负形圆形分割** 学习笔记里挂了两轮的「剩下最值得做的
  一个版式」终于落地 —— 一个深色大圆把版面切成「圆内 / 圆外」：**圆内是正形**，压反白主信息
  （小标签 + 118px 巨型词 + 单位 + 说明）；**圆外四角是负形**，压角注。它和 `cover` 一样
  **自带整版排版**（藏掉共用的双行标题），但**不自带满幅** ——「要有整版高度」和「要没有白卡」
  是两件事：`cover` 两个都自带，`circle` 只自带前者，满幅仍由 `shell:"full"` 决定。
  - **这个版式的核心矛盾是「圆越大、四角越小」**：圆要顶到卡片内边距之外（满幅时顶到画布边）、
    又不能顶出内容区、四角还得站得下角注。**CSS 给不出这个值** —— `aspect-ratio` 一旦被
    `max-height` 夹住就会悄悄变椭圆（一轴被夹时另一轴不按比例回算）。所以直径是**渲染后量一次、
    写回 `--ccd`**，角注宽同理写回 `--ccw`（和 `bigword` 的 `--wsz` 同一套路），
    **两条渲染路各算一次、`NOTE_H / PAD / CW_MIN / dx0` 四个常量对齐**。
  - 实测：卡内圆 **758px**（正好顶到卡片左右内边距）、满幅 **776px** —— **满幅反而小一圈**
    （两边各露 17px 底），因为圆再大就要吃掉四角。角注宽也随之从 208px 降到 150px，
    **角注文案要按满幅收**（卡内一行约 11 汉字、满幅只有约 7 汉字）。
  - 顺手修掉一个**会静默缩小字号**的坑：角注小标题的色条原本写成 `::before` ——
    **它不进 DOM，但会进 `Range` 的包围盒**，`fitText` 量 `.h` 时把 30px 色条 + 10px 间距
    也算进去了，字号会莫名多缩一截。改成真实的 `<i>` 元素、只量里面的 `<span class="t">`。
  - `海报探针.js` 为它加了一条专属检查：**`角注离圆=`** —— 量每个角注文字到圆边的最小距离，
    **负数就是压进圆里了**。第一版满幅样张正是被它抓出来 -46px（角注折了 3 行、顶进圆里）。
  - 又一条**纯增量**：用改动前的模板把老 18 张重渲一遍，与新版**逐字节相同**。
    样板集 18 → **20 张**（卡内 / 满幅各一张样张，仍覆盖全部 17 种版式）。
    四道自检全过：出图体检 20/20、越界探针 `over=OK`、海报探针合规、`svg校验` 20/20 ✅。
  - 新坑写进手册**第 24 条**：**「要同时看宽和高才算得出来的尺寸」CSS 给不出，只能量完写回变量**。
