#!/bin/bash
# 总览页截图：把 _总览.html 存成 _总览.png（精确高度，不留白、不截断）
#
# 用法:  ./总览截图.sh [输出目录] [宽度] [倍率]
# 默认:  ./总览截图.sh 贴图输出 1204 1.5
#
# 前置：先跑 `node 生成总览.js <数据源.json> <输出目录>` 生成 _总览.html。
#
# 为什么不直接写死 --window-size：总览的页高随张数变化（3 列一行几张、共几行），
# 写死高度不是底部留一大片底色，就是最后一行被截断。这里先让页面报出
# documentElement.scrollHeight，再按这个数字截图。
#
...
[ -f "$OUT_ABS/_总览.html" ] || { echo "找不到 $OUT_ABS/_总览.html —— 先跑 node 生成总览.js" >&2; exit 1; }
...
PROBE_FILE="$("$NODE_BIN" "$DIR/总览截图.js" prep "$OUT_ABS")" || exit 1
...
shot() {
  ...
  "$CHROME" --headless=new $1 --disable-gpu --hide-scrollbars \
    ...
    --screenshot="$tmp" \
    "file://$OUT_ABS/_总览.html" >/dev/null 2>&1 &
  pid=$!
  last=-1; stable=0
...
if [ -s "$tmp" ]; then
  mv -f "$tmp" "$OUT_ABS/_总览.png"
  echo "  _总览.png   $(( $(stat -f%z "$OUT_ABS/_总览.png") / 1024 )) KB"
  sips -g pixelWidth -g pixelHeight "$OUT_ABS/_总览.png" 2>/dev/null | tail -2 | tr -d ' ' | paste -sd' ' -
  echo "  →  $OUT_ABS/_总览.png"
else
  echo "  失败：没产出截图" >&2
  exit 1
fi
